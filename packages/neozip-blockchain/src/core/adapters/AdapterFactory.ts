...
import { V2_50Adapter } from './V2_50Adapter';
...
export function getAdapter(version: string): ContractVersionAdapter {
  ...
  if (!isVersionSupported(normalized)) {
    throw new Error(`Unsupported contract version: "${version}" (normalized: "${normalized}"). Supported versions: 2.10, 2.11, 2.50, 2.51`);
  }
