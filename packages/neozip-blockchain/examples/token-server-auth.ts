/**
 * Zipstamp Server Authentication Example (legacy name: token-server-auth)
 *
 ...
 *
 * Usage:
 *   ts-node examples/token-server-auth.ts register <email>
 *   ts-node examples/token-server-auth.ts verify <email> <code>
 *
...
async function cmdRegister(email: string): Promise<void> {
  ...
  if (result.success) {
    ...
    console.log('\nNext step:');
    console.log(`  ts-node examples/token-server-auth.ts verify ${email} <code>`);
  } else {
...
async function main(): Promise<void> {
  ...
  try {
    switch (command) {
      case 'register':
        if (!args[1]) {
          console.error('Usage: ts-node examples/token-server-auth.ts register <email>');
          process.exit(1);
        }
      ...
      case 'verify':
        if (!args[1] || !args[2]) {
          console.error('Usage: ts-node examples/token-server-auth.ts verify <email> <code>');
          process.exit(1);
        }
