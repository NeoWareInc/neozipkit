...
- Generated by `pnpm dev:build` command
...
4. Builds the project:
   - Main branch → Production build (`pnpm build:ci` → `dist/`)
   - PR branches → Development build (`pnpm dev:build` → `dev-dist/`)
...
4. Builds production version (`pnpm build:ci`)
...
3. **Dependency Issues:**
   - Run `pnpm install` locally to reproduce
...
**No manual updates needed** - just update `package.json` and run `pnpm build`.
...
1. **Always build after version changes:**
   ```bash
   pnpm version:patch
   pnpm build  # ← Required to update all version references
   ```
...
- Can be built locally with `pnpm build`
...
2. **Make Changes:**
   ...
   - Test locally with `pnpm dev:build`
   - Commit changes
...
- Verify dependencies are locked (`pnpm-lock.yaml`)
...
# Pin pnpm via root package.json "packageManager" (Corepack)
```
