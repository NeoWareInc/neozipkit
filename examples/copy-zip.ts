 ...
 * using the zipCopyUtils functions. This preserves all original properties
 * (compression, timestamps, etc.) without decompression/recompression.
 */

import { copyZipEntriesDirect } from '../src/node';
...
async function main() {
  ...
  try {
    ...
    const result = await copyZipEntriesDirect(actualSourceZip, destZip);
    ...
    console.log('─'.repeat(80));
    console.log(`Total entries copied: ${result.entries.length}`);
    ...
    console.log('   await copyZipEntriesDirect(sourceZip, destZip, (filename) => {');
    console.log('     return filename.endsWith(".txt"); // Only copy .txt files');
    console.log('   });');
