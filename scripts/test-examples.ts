...
/**
 * Test blockchain-tokenize example (may require network/config, so we test compilation)
 */
async function testBlockchainTokenize() {
  logSection('Testing blockchain-tokenize.ts');

  const examplePath = path.join(__dirname, '../examples/blockchain-tokenize.ts');

  try {
    logInfo('Running blockchain-tokenize example (will test imports and basic execution)...');
    // This example requires network access and wallet setup, so we test with a short timeout
    // and expect it to fail gracefully or show setup instructions
    const result = await executeExample(examplePath, [], 5000);
    ...
    // Check for import/module errors first - these are real failures
    if (result.error.includes('Cannot find module') || result.error.includes('MODULE_NOT_FOUND') ||
        result.error.includes('ZipkitServer') || result.error.includes('ZipkitMinterNode') ||
        result.error.includes('ZipkitVerifierNode')) {
      logError(`blockchain-tokenize.ts has import/refactoring errors: ${result.error.substring(0, 300)}`);
      if (result.error) {
        console.log('\n--- Error Output ---');
      ...
      logSuccess('blockchain-tokenize.ts executed successfully');
      results.push({
        name: 'blockchain-tokenize.ts',
        passed: true,
        duration: result.duration,
      ...
      // Otherwise, it's probably just missing config/wallet, which is expected
      logSuccess('blockchain-tokenize.ts executed (imports work, config needed)');
      results.push({
        name: 'blockchain-tokenize.ts',
        passed: true,
        duration: result.duration,
    ...
    logError(`blockchain-tokenize.ts exception: ${error instanceof Error ? error.message : String(error)}`);
    results.push({
      name: 'blockchain-tokenize.ts',
      passed: false,
      error: error instanceof Error ? error.message : String(error)
...
/**
 * Test verify-tokenized-zip example
 */
async function testVerifyTokenizedZip() {
  logSection('Testing verify-tokenized-zip.ts');

  const examplePath = path.join(__dirname, '../examples/verify-tokenized-zip.ts');
  const testZip = path.join(tempDir, 'nonexistent.zip');

  try {
    logInfo('Running verify-tokenized-zip example (will test error handling)...');
    const result = await executeExample(examplePath, [testZip], 10000);
    ...
    // This example should handle missing files gracefully
    if (result.success || result.error.includes('not found') || result.output.includes('Error') ||
        result.output.includes('ZIP file not found')) {
      logSuccess('verify-tokenized-zip.ts executed (handled missing file or verified)');
      results.push({
        name: 'verify-tokenized-zip.ts',
        passed: true,
        duration: result.duration,
        output: result.output
      });
    } else if (result.error.includes('Cannot find module') || result.error.includes('MODULE_NOT_FOUND') ||
               result.error.includes('ZipkitServer') || result.error.includes('ZipkitVerifierNode')) {
      logError(`verify-tokenized-zip.ts has import/refactoring errors: ${result.error.substring(0, 300)}`);
      if (result.error) {
        console.log('\n--- Error Output ---');
        console.log(result.error.substring(0, 500));
        console.log('--- End Error ---\n');
      ...
      logWarning(`verify-tokenized-zip.ts: ${result.error || 'Unexpected result'}`);
      results.push({
        name: 'verify-tokenized-zip.ts',
        passed: true, // Pass if it's just a missing file
        duration: result.duration,
        output: result.output,
        error: result.error
      });
    ...
    logError(`verify-tokenized-zip.ts exception: ${error instanceof Error ? error.message : String(error)}`);
    results.push({
      name: 'verify-tokenized-zip.ts',
      passed: false,
      error: error instanceof Error ? error.message : String(error)
...
async function runTests() {
  ...
  try {
    ...
    await testBlockchainTokenize();
    await testVerifyTokenizedZip();
